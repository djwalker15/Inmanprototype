...
// ── Seed data for InMan ──
// Migrated from mock-data.ts with Location→Space renaming,
// added premises/area root nodes, consolidated unit_types
...
export async function seedData() {
  ...
  const catKeys = categories.map(c => `category:${c.category_id}`);
  await kv.mset(catKeys, categories);
  await kv.set("meta:next_category_id", 13);

  // ═══════ SPACES (55 = 2 new root nodes + 53 migrated) ═══════
  // New root nodes: My House (premises) and Kitchen (area)
  // Old zones get parent_id = 2 (Kitchen)
  // Old unit_types cabinet/drawer/compartment → container
  const spaces = [
    // Root nodes (NEW)
    ...
    // Zones — now children of Kitchen (id=2)
  ...
  const spaceKeys = spaces.map(s => `space:${s.space_id}`);
  await kv.mset(spaceKeys, spaces);
  await kv.set("meta:next_space_id", 56);
  ...
  // Batch insert items in groups of 20 to avoid payload limits
  for (let i = 0; i < items.length; i += 20) {
    const batch = items.slice(i, i + 20);
    const keys = batch.map(item => `item:${item.item_id}`);
    await kv.mset(keys, batch);
  }
  await kv.set("meta:next_item_id", 166);
}
